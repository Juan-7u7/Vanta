...
-db/: dumps o scripts de base de datos locales (no se usa en runtime).
-dist/: build generado por Vite (salida de producción).
-docs/: documentación del proyecto (setup, arquitectura, operaciones, dev-notes, etc.).
-node_modules/: dependencias instaladas (autogenerado).
-public/: assets estáticos servidos tal cual (logos, íconos).
-scripts/: utilidades de mantenimiento/seed; ej. seedPlantillas.ts para poblar esquemas en Supabase.
-src/: código fuente de la app (React/TS).
-assets/: imágenes o recursos usados en UI.
-components/: componentes reutilizables y modales (incluye Sidebar, modales de edición, etc.).
-config/: configuraciones/constantes (plantillas de esquemas, etc.).
-context/: providers (AuthContext).
-lib/: clientes externos (Supabase).
-pages/: vistas principales (Dashboard, Escalones, Imprimir COVAS, etc.).
-services/: lógica de generación de datos/PDF (CovasGenerator, CovasPDFGenerator).
-types/: tipos/definiciones TS.
-utils/: helpers y lógica financiera (covasLogic*).
-tmp/: archivos temporales (no esenciales).
-.env: variables de entorno locales (no versionar en prod).
-esquema.json / components.json: configuraciones auxiliares del proyecto.
-README.md / README_LOGICA_FINANCIERA.md / DOCS_COLABORADOR.md: documentación principal y lógica financiera.
-package.json: dependencias y scripts npm.
-tsconfig.json*: configuración TypeScript (app/node).
-vercel.json: rewrites y config para despliegue en Vercel.
-vite.config.ts: configuración de Vite (build/dev).
