...
const inputClass = `
  ...
  focus:ring-2 focus:ring-violet-500/40 focus:border-violet-500/40
`;

const labelClass = 'block text-[11px] font-bold text-gray-400 uppercase tracking-wider mb-1.5';

export default function AlcanceModal({ isOpen, onClose, onSuccess, initialData }: AlcanceModalProps) {
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-10 sm:pt-16">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-[#0f1117] border border-gray-100 dark:border-white/10 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-7 pt-7 pb-5 border-b border-gray-100 dark:border-white/5">
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">
              {isEditing ? 'Editar Alcance' : 'Nuevo Alcance'}
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">
              {isEditing ? 'Modifica los valores de alcance real' : 'Registra el alcance real mensual'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-7 py-6 space-y-5">
          ...
          {/* Valores de alcance por mes */}
          <div>
            <label className={labelClass}>Alcance Real por Mes</label>
            <p className="text-[10px] text-gray-400 mb-3">
              ...
              Ejemplos: <code className="bg-gray-100 dark:bg-white/10 px-1 rounded">12,500.00</code>{' '}
              · <code className="bg-gray-100 dark:bg-white/10 px-1 rounded">78.50</code>
            </p>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-violet-600 hover:bg-violet-500 text-white font-bold text-sm transition-all disabled:opacity-60 shadow-lg shadow-violet-500/20"
            >
              {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
              {loading ? 'Guardando...' : isEditing ? 'Guardar Cambios' : 'Crear Alcance'}
            </button>
          </div>
        </form>
      </div>
