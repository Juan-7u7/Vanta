...
export default function SalarioModal({ isOpen, onClose, onSuccess, initialData }: SalarioModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Solo campos de salarios_mensuales
      const payload = {
    ...
    <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 pt-10 sm:pt-16">
      ...
      <div className="relative w-full max-w-2xl bg-white dark:bg-[#0f1117] border border-gray-100 dark:border-white/10 rounded-[32px] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between px-7 pt-7 pb-5 border-b border-gray-100 dark:border-white/5">
          <div>
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">
              {isEditing ? 'Editar Salario' : 'Nuevo Salario'}
            </h2>
            <p className="text-xs text-gray-400 mt-0.5">
              {isEditing ? 'Modifica los montos mensuales' : 'Registra los salarios mensuales'}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-7 py-6 space-y-5">
          ...
          {/* Salarios mensuales */}
          <div>
            <label className={labelClass}>Salarios Mensuales</label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm transition-all disabled:opacity-60 shadow-lg shadow-emerald-500/20"
            >
              {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
              {loading ? 'Guardando...' : isEditing ? 'Guardar Cambios' : 'Crear Salario'}
            </button>
          </div>
        </form>
      </div>
